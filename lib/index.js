...
module.exports = function () {
  return {
    ...
    uploadTrack: function uploadTrack(cameraTrackId, serial, geoData, socket) {
      _async2.default.eachSeries(geoData, function (data, callback) {
        (0, _uploader2.default)(_camera, '/home/pi/wawycam/api/snap/' + data.media.file, function (percent) {
          socket.emit('sync:progress', { media: data.media.file, percent: percent });
          if (percent === 'uploaded') {
            apolloClient.mutate({
              mutation: _gql.mutation.POST_TRACK_ID,
              variables: {
                cameraTrackId: cameraTrackId,
                camera: serial,
                geoData: {
              ...
              _logger2.default.info(result.data);
              callback();
            }).catch(function (error) {
              return _logger2.default.error(error);
            });
